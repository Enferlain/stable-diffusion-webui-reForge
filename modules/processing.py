...
def apply_overlay_precise(image: Image.Image, paste_loc: tuple[int], overlay: tuple[Image.Image, np.ndarray]):
    ...
    print(f"Precise overlay dtypes: overlay={overlay_rgb.dtype}, image={image_rgb.dtype}, mask={_mask.dtype}")
...
def apply_overlay(image: Image.Image, paste_loc: tuple[int], overlay: Any):
    if overlay is None:
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        ...
        if image_mask is not None:
            ...
            if opts.img2img_inpaint_precise_mask and self.mask_blur_x * self.mask_blur_y > 0:
                print("Applying high precision mask blur...")
                _np_mask = np.array(image_mask).astype(np.float32) / 255.0
                print(f"Mask dtype for blurring: {_np_mask.dtype}")
                kernel_size = 2 * int(2.5 * self.mask_blur_x + 0.5) + 1
